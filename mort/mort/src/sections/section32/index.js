const Section32 = () => {
    return(
        <div className=" bg-black text-white font-economica
        xxs:h-[135vh]
        xs:h-[150vh]
        sm:text-lg sm:h-[155vh]
        md:text-2xl md:h-[125vh]
        lg:text-lg lg:p-4 lg:h-[200vh]
        xl:h-[220vh]">



<h3 className="text-center p-2">Please, make this your fight. Ashley needs your help now more than ever. She is only 22 years old. Right now as you read these words she languishes in a Thai prison cell where she does not even understand the words spoken for a crime she had nothing to do with. She has not been heard from by anyone for over two months. She is not allowed books, letters, or calls.</h3>


<h1 className="text-center p-2">Won't you please help Ashley Oosthuizen? </h1>

<h3 className="text-center p-2">Please, at least share this website and her story and if you are able to do more, she would be most grateful if you did. Be well assured that she is certainly worth it. Thank you for your help, compassion, and time.</h3>
<img src="./images/Ashley5.jpg" alt="Sergey Requesting Payment" srcset="" 
  className="xxs:w-[95vw]  xxs:mx-auto xxs:pt-2"
/>

<h3  className="text-center p-2 lg:pt-10">Donate to Ashley's legal fund soon, it is being set up and run by her younger brother, Warren Oosthuizen.</h3>
<div className="flex justify-around">


</div>

<h1 className="text-center text-5xl text-yellow-500 xxs:pt-5 xs:pt-14
lg:pt-14">FREE ASHLEY</h1>
</div>
    )
}

export default Section32