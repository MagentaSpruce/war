const Section30 = () => {
    return(
        <div className=" bg-black text-white font-economica
        xxs:h-[80vh]
        xs:h-[90vh]">

<h3 className="text-center p-2">Please consider donating to Ashley's go fund below.</h3>

<h3 className="text-center p-2">Please consider donating to Ashley's Ethereum legal fund below.</h3>

<h3 className="text-center p-2">Please consider purchasing a copy of 'The Shepherd - A Bronze Age Tale.' 95% of all author profits will go to Ashley's legal fund.</h3>

<div className="flex flex-col">
<h3 className="text-center p-2">Please consider becoming a subscribing member to Ashley Capital & DeFi, a crypto consulting and wealth management company that is being started in her name and honor. 95% of all profits made will go to her legal fund.</h3>

<a href="www.ashleydefi.com" className="mx-auto underline-offset-4 underline cursor-pointer">Ashley Capital & DeFi</a>
</div>

<h3 className="text-center p-2">Please, for Ashley, do at least one of these many things to help spread her story and garner support for her cause. </h3>

<h3 className="text-center p-2">With the help of people like you, we can get her back to us.</h3>




</div>
    )
}

export default Section30